...
logger.add_hook(lambda x: logger.plot(x, "Encoding Loss", opts=dict(ymin=0)), 
	feature='loss', freq=20,
)
...
def compute_changed_images(images, perturbation, std_weights, epsilon=EPSILON):
	...
	perturbation_zc = perturbation_w2/perturbation_w2.view(perturbation_w2.shape[0], -1)\
			.norm(2, dim=1, keepdim=True).unsqueeze(2).unsqueeze(2)\
			.expand_as(perturbation_w2)*epsilon*(perturbation_w2[0].nelement()**0.5)
...
"""
def encode_binary(images, targets, model=DecodingModel(), n=None,
					max_iter=500, verbose=False, perturbation=None, use_weighting=False):
	
	if n is not None: 
		if verbose: print (f"Changing distribution size: {model.n} -> {n}")
		n, model.n = (model.n, n)
	...
	if n is not None: 
		if verbose: print (f"Fixing distribution size: {model.n} -> {n}")
		n, model.n = (model.n, n)
...
def encode(image, out, target=binary.str(binary.random(TARGET_SIZE)), n=96,
			model=None, max_iter=500, use_weighting=True, perturbation_out=None):
